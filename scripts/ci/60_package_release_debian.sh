...
: "${DEBIAN_RELEASE:?missing DEBIAN_RELEASE}"
...
- Distribution: \`Debian ${DEBIAN_RELEASE}\`
...
TAG_NAME="debian${DEBIAN_RELEASE}-${KREL}$(if [[ "$BUILD_EL2" == "true" ]]; then printf -- '-el2'; fi)-$(date -u +%Y%m%d%H%M%S)"
